        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="mt-4 flex items-end gap-4">
              ...
              {product.status === 'published' && product.quantity > 0 ? <span className="px-2 py-1 bg-emerald-100 text-emerald-700 text-xs font-semibold rounded-full mb-1">In Stock ({product.quantity})</span> : <span className="px-2 py-1 bg-red-100 text-red-700 text-xs font-semibold rounded-full mb-1">Out of Stock</span>}
            </div>
          ...
          <div className="flex items-center gap-4 text-sm text-slate-500"><div className="flex items-center gap-1"><MapPin className="w-4 h-4" /> {product.location.address || 'Location hidden'}</div></div>
          <div className="pt-6 border-t border-slate-100 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button onClick={handleSaveWishlist} disabled={savingWishlist} className="px-6 py-4 rounded-xl border border-slate-200 font-medium text-slate-700 hover:bg-slate-50 transition-colors flex items-center justify-center gap-2">{savingWishlist ? <Loader2 className="w-5 h-5 animate-spin" /> : <Heart className="w-5 h-5" />} Save</button>
            </div>
